...
Publisher SHALL 对每条有效 diff position 的 Risk 以行级内联评论（`DraftPullRequestReviewComment`）形式发布，而非仅在 body 中以文本列表展示。

每条内联评论 MUST 包含：
- `path`: Risk.File，相对文件路径
- `position`: 通过 `FileLineToPosition` 从 diff hunk 计算出的位置值
...
- **THEN** 该 risk 以内联 `DraftPullRequestReviewComment` 形式发布，不在 body 中重复出现
